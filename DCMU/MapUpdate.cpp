...
void calLMD(GeoPoint* pt, int k, double kNNThresholdM)
{
	...
	allPtIndex.kNN(pt, k, kNNThresholdM, kNNSet);
	double lmd = 0;
...
void outlierValidation(GeoPoint* pt, int gridRange, double supportRatio)
{
	...
	pair<int, int> rolCol = allPtIndex.getRowCol(pt);
	...
	if (row1 < 0) row1 = 0;
	if (row2 >= allPtIndex.gridHeight) row2 = allPtIndex.gridHeight - 1;
	if (col1 < 0) col1 = 0;
	if (col2 >= allPtIndex.gridWidth) col2 = allPtIndex.gridWidth - 1;
	int currentGridCount = allPtIndex.grid[rowPt][colPt]->size();
	for (int row = row1; row <= row2; row++)
	{
		for (int col = col1; col <= col2; col++)
		{
			if (allPtIndex.grid[row][col]->size() / currentGridCount > supportRatio)
			{
				pt->isOutlier = 1;
				return;
			}
	...
	for (int row = row1; row <= row2; row++)
	{
		for (int col = col1; col <= col2; col++)
		{
			if (row == rowPt && col == colPt)
				continue;
			for each (GeoPoint* pt in *(allPtIndex.grid[row][col]))
			{
				if (pt->isOutlier == 1)
					outlierCount++;
				else
...
void calDir(GeoPoint* p0, double angleStep, double d, double l)
{
	...
	vector<GeoPoint*> nearPts;
	allPtIndex.getNearPts(p0, sqrt(d / 2 * d / 2 + l*l), nearPts);
	vector<int> countVec;
...
void getPtsNearCluster(Cluster* cluster, double distThresM, double angleThres, vector<GeoPoint*>& nearPts)
{
	...
	//////////////////////////////////////////////////////////////////////////
	nearPts.clear();
	pair<int, int> rowCol_s = allPtIndex.getRowCol(new GeoPoint(cluster->area.minLat, cluster->area.minLon));
	pair<int, int> rowCol_e = allPtIndex.getRowCol(new GeoPoint(cluster->area.maxLat, cluster->area.maxLon));
	rowCol_s.first--;
	rowCol_s.second--;
	rowCol_e.first++;
	rowCol_e.second++;
	if (rowCol_s.first < 0) rowCol_s.first = 0;
	if (rowCol_e.first >= allPtIndex.gridHeight) rowCol_e.first = allPtIndex.gridHeight - 1;
	if (rowCol_s.second < 0) rowCol_s.second = 0;
	if (rowCol_e.second >= allPtIndex.gridWidth) rowCol_e.second = allPtIndex.gridWidth - 1;
...
bool fetch(Cluster* cluster, double distThresM, double angleThres)
{
	...
	for (int i = 0; i < nearPts.size(); i++)
	{
		...
		allPtIndex.getNearPts(currentPt, 1, ptsNearCurrentPt);
